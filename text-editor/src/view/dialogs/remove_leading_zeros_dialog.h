...
#include "domain/commands/remove_leading_zeros.h"
#include "view/text_space_view.h"
#include "view/ui_primitives/submit_command_dialog.h"

namespace text_editor {

	class RemoveLeadingZerosDialog : public CommandDialog {
	...
	public:
		...
		virtual void on_process(bool* opened) {
			...
			bool submited = submit_command_dialog("Remove Leading Zeros", opened, _on_submition);

			if (submited) {
				RemoveLeadingZerosData data = {
					_start, _end
				};
				RemoveLeadingZerosCommand command = { data };
